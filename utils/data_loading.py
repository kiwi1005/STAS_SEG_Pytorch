...
class BasicDataset(Dataset):
    ...
    def dataAug(img,mask):
	    ...
	    A.CLAHE(clip_limit=2.0, tile_grid_size=(16, 16), always_apply=False, p=0.8),
	    
            A.GaussianBlur(blur_limit=(1, 3), sigma_limit=0, always_apply=False, p=0.5),
	    A.Sharpen(alpha=(0.3, 0.6), lightness=(0.5, 0.7), always_apply=False,p=0.5),
	    A.CropNonEmptyMaskIfExists (672, 672, ignore_values=[255], ignore_channels=[1], always_apply=True, p=0.8),
    ...
    def predict(pil_img, scale, is_mask):
        transform = A.Compose([
            A.PadIfNeeded(min_height=960, min_width=1728, always_apply=True, border_mode=0),
         ])
